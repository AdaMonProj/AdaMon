...
Vector_Bloom_Filter::Vector_Bloom_Filter(uint32_t mem, double cmratio): AdaMon_bm(mem * cmratio) {
    if(cmratio == 0){
        use_AdaMon = false;
        cout << "AdaMon not used" << endl;
    }   
...
void Vector_Bloom_Filter::process_packet(string srcip, array<uint8_t,4> srcip_tuple, string dstip){
    ...
    if (use_AdaMon) {
        bool full_flag = AdaMon_bm.process_packet(hash_flowid, hash_element);
        if (full_flag == false)
            return;
    } 
...
void Vector_Bloom_Filter::Detect_Superpoint(vector<IdSpread>* superspreaders) {
    ...
    for(size_t row = 0;row<IP.rows;row++) {
        ...
        if(zero_num <= Z) {
            ...
            //Filter out false positives with AdaMon Bitmap Array 
            if (use_AdaMon){
                // array<uint64_t,2> hash_flowid = str_hash128(ipstr, HASH_SEED_1);
                bool full_flag = AdaMon_bm.check_flow_full(hash_flowid);
                if (full_flag == true){
